...
from src.submissions.results import ResultBuilder
...
def main():
    ...
    corpus = np.ndarray((1,1))
    corpus_labels = None
    search_faiss = FaissRetriever(embeddings_size=corpus.shape[1], metric=metric, use_gpu=False)
    search_faiss.add_to_index(corpus, labels=corpus_labels)
